...
const warningBadge = (level: StudentWarningLevel) => level === 'other' ? { label: '其他个案', emoji: '🟣', color: '#8b5cf6', weight: 2 } : level === 'none' ? { label: crisisConfig.levelLabels.normal_label, emoji: '🟢', color: '#64748b', weight: 0 } : crisisConfig.getStudentWarningBadge(level)
const warningOptions = computed<Array<{ value: WarningFilter; label: string }>>(() => [{ value: 'all', label: '全部等级' }, { value: 'red', label: `${warningBadge('red').emoji} ${warningBadge('red').label} / 红色` }, { value: 'orange', label: `${warningBadge('orange').emoji} ${warningBadge('orange').label} / 橙色` }, { value: 'yellow', label: `${warningBadge('yellow').emoji} ${warningBadge('yellow').label} / 黄色` }, { value: 'none', label: `⚪ ${warningBadge('none').label} / 普通个案` }, { value: 'other', label: '🟣 其他个案' }])
const warningLabel = (level: StudentWarningLevel) => `${warningBadge(level).emoji} ${warningBadge(level).label}`
