...
import { MAX_TEACHING_MATERIAL_BYTES, prepareInlineAttachment } from '../../services/storageBoundary'
...
async function attachFile(event: Event) {
  ...
  if (!file) return
  try {
    const prepared = await prepareInlineAttachment(file, { maxBytes: MAX_TEACHING_MATERIAL_BYTES, compressImages: true })
    form.attachment = { name: file.name, type: file.type || 'application/octet-stream', dataUrl: prepared.dataUrl }
  } catch (error) { alert(error instanceof Error ? error.message : '附件读取失败，请重新选择文件。') }
}
async function save() {
