...
const emit = defineEmits<{ open: [plan: LessonPlan, schedule: WeeklySchedule | null, classOptions?: ClassOption[]]; requestUnit: [planId?: string] }>()
...
function classesForUnit(unit: TeachingProgressUnit) { return props.classes.filter((item) => unit.targetGrades.includes(item.grade)) }
    ...
    <div v-else class="mt-5 space-y-4">
      <article v-for="item in visibleUnits" :key="item.unit.id" class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header class="flex flex-wrap items-center gap-3 px-4 py-3">
          ...
          <button type="button" class="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs text-slate-500 hover:bg-slate-100 hover:text-slate-800" @click.stop="emit('open', item.plan, null, classesForUnit(item.unit))"><Eye :size="14" />教案速览</button>
