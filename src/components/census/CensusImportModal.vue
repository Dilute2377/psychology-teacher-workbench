...
const duplicateStudentNos = computed(() => {
  const counts = new Map<string, number>()
  parsedRows.value.forEach((row) => counts.set(row.studentNo, (counts.get(row.studentNo) ?? 0) + 1))
  return new Set([...counts.entries()].filter(([, count]) => count > 1).map(([studentNo]) => studentNo))
})
...
async function confirm() {
  ...
  if (duplicateStudentNos.value.size) { message.value = `导入数据存在重复学号：${[...duplicateStudentNos.value].join('、')}。同名学生可以保留，但同一学号只能出现一次。`; return }
