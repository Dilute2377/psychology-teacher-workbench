...
async function addTerm() {
  ...
  const matched = academicYear.match(/^(\d{4})-(\d{4})$/)
  if (!matched || Number(matched[2]) !== Number(matched[1]) + 1) {
    message.value = '学年请按“2026-2027”的格式填写，且后一年度应比前一年大 1。'
    return
  }
  if (!form.startDate || !form.endDate || form.startDate >= form.endDate) {
    message.value = '请填写有效的开始与结束日期，且结束日期需晚于开始日期。'
    return
  }
  try { await termStore.addTerm({ ...form, academicYear }); await refresh(); resetForm(); message.value = '新学期已保存到本地数据库。' } catch (error) { message.value = error instanceof Error ? error.message : '保存学期失败，请检查填写内容。' }
}
