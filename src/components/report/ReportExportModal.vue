...
const riskSummary = computed(() => report.value?.riskDistribution.filter((item) => item.key !== 'normal').map((item) => `${item.emoji} ${item.label} ${item.count}人`).join(' · ') || '暂无异常评级学生')
const highRiskBandLabel = computed(() => crisisConfig.getHighRiskLevel() === 'level_1' ? crisisConfig.getLevelBadge('level_1').label : crisisConfig.getLevelBadge('level_3').label)
...
onMounted(() => { crisisConfig.load(); window.addEventListener('keydown', handleKeydown); void loadReport() })
          ...
          <template v-else-if="report">
            ...
            <section class="mt-6 print-no-break">
              ...
              <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <article class="rounded-xl border border-rose-100 bg-rose-50/70 p-3"><p class="text-[11px] font-medium text-rose-700">非正常评级在册</p><p class="mt-2 text-2xl font-bold tabular-nums text-slate-800">{{ report.activeRiskCount }}<span class="ml-1 text-xs font-medium text-slate-500">人</span></p><p class="mt-1 text-[10px] text-slate-500">{{ riskSummary }}</p></article>
